<script lang="ts">
	...
	function triggerBurst(changeSize: number) {
		...
		const particles = Array.from({ length: 10 + intensity * 4 }, (_, index) => {
			...
			const colorClass = index % 3 === 0 ? 'bg-[#FDBA74]' : index % 2 === 0 ? 'bg-[#FB7185]' : 'bg-[#C084FC]';
	...
	let iconSize = $derived(40 + Math.min(streak, 30) * 1.2);
</script>

<section
	class="relative overflow-hidden rounded-[40px] border border-white/80 bg-[#fff8fb] p-6 shadow-[0_18px_48px_-18px_rgba(15,23,42,0.12)] backdrop-blur-xl"
>
	<div
		class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(253,164,175,0.20),transparent_32%),radial-gradient(circle_at_bottom_right,rgba(192,132,252,0.14),transparent_28%)]"
	></div>
	...
	<div class="relative grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(220px,0.8fr)] lg:items-center">
		<div class="min-w-0">
			<div class="mt-2 flex items-end gap-3">
				<span class="text-[72px] leading-none font-black tracking-tighter text-[#111827] drop-shadow-sm"
					>{streak}</span
				>
			...
			<p class="mt-1 text-xs font-bold text-gray-400">
				{streak > 0
					? 'Draw together every day to keep the streak burning! 🔥'
					: 'Save a cute doodle with your partner today to start! ✨'}
			</p>
		</div>

		<div class="relative flex justify-center lg:justify-end">
			<div
				class="relative flex h-48 w-48 items-center justify-center rounded-full bg-white/70 border border-white shadow-[0_20px_48px_-24px_rgba(0,0,0,0.35)]"
			>
				...
				<div class="absolute inset-7 rounded-full bg-linear-to-br from-[#FFF1F5] to-[#F3E8FF] shadow-inner"></div>
				...
				{#each Array.from({ length: maxSegments }, (_, index) => index) as index (index)}
					<div
						class={`absolute h-2.5 w-2.5 rounded-full transition-all duration-500 ${
							index < activeSegments
								? 'bg-[#FB7185] shadow-[0_0_12px_rgba(251,113,133,0.6)]'
								: 'bg-[#FBCFE8]'
						}`}
				...
				<div class="relative z-10 flex flex-col items-center justify-center text-center">
					<svg
						class="{fireColorClass} drop-shadow-sm transition-all duration-300"
	...
	<div
		class="relative mt-6 grid grid-cols-7 gap-2 rounded-[28px] bg-white/70 border border-white/80 px-4 py-4 shadow-[0_10px_28px_-18px_rgba(0,0,0,0.18)]"
	>
		{#each Array.from({ length: maxSegments }, (_, index) => index) as index (index)}
			<div
				class={`h-2.5 rounded-full transition-all duration-500 ${
					index < activeSegments ? 'bg-linear-to-r from-[#FB7185] to-[#C084FC]' : 'bg-[#FBCFE8]/70'
				}`}
			></div>
		{/each}
	</div>
