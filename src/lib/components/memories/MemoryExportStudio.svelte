...
{#if isOpen}
    <div class="fixed inset-0 z-[999999] overflow-y-auto bg-slate-950/72 p-4 backdrop-blur-xl" transition:fade={{ duration: 180 }}>
        <div class="mx-auto flex min-h-full w-full max-w-7xl items-center justify-center py-6">
            <div transition:scale={{ duration: 220, start: 0.96 }} class="grid w-full gap-5 rounded-[32px] border border-white/60 bg-white/92 p-4 shadow-[0_24px_90px_-20px_rgba(15,23,42,0.45)] backdrop-blur-2xl lg:grid-cols-[1.15fr_0.85fr] lg:p-5">
                <section class="rounded-[28px] bg-[#FFF7ED] p-4 sm:p-5">
                    <div class="mb-4 flex items-center justify-between gap-4">
                        <div>
                            <p class="text-[11px] font-black uppercase tracking-[0.28em] text-[#FDA4AF]">Memory export studio</p>
                            <h2 class="mt-1 text-2xl font-black tracking-tight text-gray-900">Instagramable share kit</h2>
                            <p class="mt-1 text-sm font-medium text-gray-500">PNG poster, GIF loop, or video export from your media story.</p>
                        </div>
                        <button type="button" onclick={close} class="flex h-10 w-10 items-center justify-center rounded-full border border-white/70 bg-white text-gray-900 shadow-sm transition-transform active:scale-90" aria-label="Close export studio">
                    ...
                    <div class="mb-4 flex flex-wrap gap-2">
                        <span class="rounded-full bg-white px-3 py-1 text-[11px] font-black uppercase tracking-widest text-gray-600 shadow-sm">{items.length} media</span>
                        <span class="rounded-full bg-white px-3 py-1 text-[11px] font-black uppercase tracking-widest text-gray-600 shadow-sm">{sizeLabel}</span>
                        <span class="rounded-full bg-white px-3 py-1 text-[11px] font-black uppercase tracking-widest text-gray-600 shadow-sm">{themeName}</span>
                        <span class="rounded-full bg-white px-3 py-1 text-[11px] font-black uppercase tracking-widest text-gray-600 shadow-sm">{themeCaption}</span>
                    </div>

                    <div class="relative mx-auto overflow-hidden rounded-[30px] border border-white/80 shadow-[0_14px_40px_-14px_rgba(15,23,42,0.2)]" style={`aspect-ratio: ${getMemoryExportSize(selectedAspect).width} / ${getMemoryExportSize(selectedAspect).height}; max-height: ${previewHeight};`}>
                        <div class={`absolute inset-0 bg-gradient-to-br ${themeOptions.find((option) => option.value === selectedTheme)?.gradient}`}></div>
                        <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.40),_transparent_40%),radial-gradient(circle_at_bottom_right,_rgba(255,255,255,0.18),_transparent_30%)]"></div>
                        {#if selectedMedia}
                            <div class="absolute inset-0 p-4 sm:p-5">
                                <div class="absolute left-4 top-4 z-10 flex flex-wrap gap-2">
                                    <span class="rounded-full bg-black/20 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-white backdrop-blur-md">Twoly</span>
                                    <span class="rounded-full bg-black/20 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-white backdrop-blur-md">{sizeLabel}</span>
                                </div>

                                <div class="absolute inset-x-4 top-12 bottom-28 overflow-hidden rounded-[24px] bg-black/10 shadow-[0_12px_40px_-18px_rgba(0,0,0,0.5)]">
                                    <MediaPreview
                                ...
                                <div class="absolute inset-x-4 bottom-4 rounded-[22px] border border-white/25 bg-black/30 p-4 text-white backdrop-blur-xl shadow-[0_8px_30px_-12px_rgba(0,0,0,0.45)]">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="min-w-0 flex-1">
                                            <p class="text-[11px] font-black uppercase tracking-[0.3em] text-white/60">{memoryDate ? memoryDate : 'Memory story'}</p>
                                            <h3 class="mt-1 truncate text-xl font-black tracking-tight">{title}</h3>
                                        ...
                                        <div class="flex shrink-0 flex-col items-end gap-2 text-right">
                                            <span class="rounded-full bg-white/15 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-white/80">
                    ...
                    <div class="mt-4 overflow-hidden rounded-[24px] border border-white/70 bg-white/60 p-3 shadow-sm">
                        ...
                        <div class="flex gap-3 overflow-x-auto pb-1 hide-scrollbar">
                            {#each items as item, index (item.id)}
                                <button
                                    ...
                                    class="group relative h-22 w-16 shrink-0 overflow-hidden rounded-2xl border bg-gray-100 transition-all active:scale-95 {selectedPosterIndex === index ? 'border-[#FDA4AF] shadow-[0_10px_25px_-10px_rgba(253,164,175,0.55)] ring-4 ring-[#FDA4AF]/15' : 'border-white/80'}"
                ...
                <aside class="rounded-[28px] bg-[#FFF7ED] p-4 sm:p-5">
                        ...
                        <div class="mt-3 grid gap-2">
                            {#each formatOptions as option (option.value)}
                                <button
                                    ...
                                    class="w-full rounded-2xl border px-4 py-3 text-left transition-all active:scale-[0.99] {selectedFormat === option.value ? 'border-[#FDA4AF] bg-[#FDA4AF]/10 shadow-[0_10px_25px_-12px_rgba(253,164,175,0.55)]' : 'border-gray-100 bg-white hover:border-[#FDA4AF]/30'}"
                                >
                                    <div class="flex items-start justify-between gap-4">
                                        <div>
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-2">
                            {#each aspectOptions as option (option.value)}
                                <button
                                    ...
                                    class="rounded-2xl border px-3 py-3 text-left transition-all active:scale-[0.99] {selectedAspect === option.value ? 'border-[#FDA4AF] bg-[#FDA4AF]/10 shadow-[0_10px_25px_-12px_rgba(253,164,175,0.45)]' : 'border-gray-100 bg-white hover:border-[#FDA4AF]/30'}"
                                >
                                    <p class="text-sm font-black text-gray-900">{option.label}</p>
                                    <p class="mt-0.5 text-[11px] font-medium text-gray-500">{option.description}</p>
                                </button>
                        ...
                        <div class="mt-3 grid gap-2">
                            {#each themeOptions as option (option.value)}
                                <button
                                    ...
                                    class="w-full rounded-2xl border px-4 py-3 text-left transition-all active:scale-[0.99] {selectedTheme === option.value ? 'border-[#FDA4AF] bg-[#FDA4AF]/10 shadow-[0_10px_25px_-12px_rgba(253,164,175,0.55)]' : 'border-gray-100 bg-white hover:border-[#FDA4AF]/30'}"
                                >
                    ...
                    <div class="mt-5 rounded-[24px] border border-[#FDA4AF]/20 bg-white/80 p-4 shadow-sm">
                    ...
                    {#if exportMessage}
                        <div class="mt-4 rounded-[20px] border border-[#FDA4AF]/20 bg-[#FDA4AF]/10 px-4 py-3 text-sm font-medium text-gray-700">
                            {exportMessage}
                        </div>
                    {/if}

                    <div class="mt-5 grid gap-2 sm:grid-cols-2">
                        <Button variant="secondary" class="w-full" onclick={close} disabled={isExporting}>Cancel</Button>
                        <Button class="w-full" onclick={handleExport} isLoading={isExporting} disabled={!items.length}>
                            {exportButtonText()}
                        </Button>
                    </div>
                </aside>
